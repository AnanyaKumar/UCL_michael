...
class FMOW(ContinualDataset):
    ...
    def get_data_loaders(self, args):
        task_train_data = deepcopy.copy(self.train_data)
        task_train_data.indices = self.train_dataset.where(self.train_dataset['region'] == self.REGION_ORDER[self.i]).index.values

        task_val_data = deepcopy.copy(self.val_data)
        task_val_data.indices = self.val_dataset.where(self.val_dataset['region'] == self.REGION_ORDER[self.i]).index.values

        task_test_data = deepcopy.copy(self.test_data)
        task_test_data.indices = self.test_dataset.where(self.test_dataset['region'] == self.REGION_ORDER[self.i]).index.values

        train_loader = get_train_loader("standard", task_train_data, batch_size=self.args.train.batch_size, shuffle=True, num_workers=16)
        memory_loader = get_eval_loader("standard", task_val_data,batch_size=self.args.train.batch_size, shuffle=False, num_workers=0)
        test_loader = get_eval_loader("standard", task_test_data, batch_size=self.args.train.batch_size, shuffle=False, num_workers=0)
