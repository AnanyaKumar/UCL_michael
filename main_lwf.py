...
def main(device, args):
    ...
    for t in range(dataset.N_TASKS):
      ...
      if hasattr(model, 'begin_task'):
          model.begin_task(train_loaders[t])
      ...
      for epoch in global_progress:
        ...
        for idx, data in enumerate(local_progress):
            if hasattr(train_loaders[t].dataset, 'logits'):
                (images1, images2, notaug_images), labels, logits = data
                print(images1.shape, images2.shape, notaug_images.shape, labels.shape, logits.shape)
                data_dict = model.observe(images1, labels, notaug_images, logits)
            else:
        ...
        if (epoch + 1) == args.train.stop_at_epoch:
            ...
            if args.train.knn_monitor:
                knn_acc_list = []
                for i in eval_tids:
                    acc, acc_mask = knn_monitor(model.net.backbone, dataset, dataset.memory_loaders[i], dataset.test_loaders[i],
